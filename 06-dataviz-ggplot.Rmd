...
> If you want to see more examples of geometries and their customizations, please check out the [_R Graph Gallery_](http://r-graph-gallery.com/).
...
ggplot(penguins)+
  geom_point(aes(x=bill_length_mm, 
                 ...
                 shape=sex,
                 color=sex), size=5)+
  labs(title="Shape and color controlled by data, size set outside aesthetics")

ggplot(penguins)+
  geom_point(aes(x=bill_length_mm, y=flipper_length_mm,
                 color=species),
             size=4, shape=3)+
  labs(title="Color controlled by data, size and shape set outside aesthetics")
```
...
See __ for a list of R color names.
...
ggplot(mtcars) + 
  geom_point(aes(mpg, disp)) +
  theme_wsj()+
  ggtitle('Wall Street Journal: theme_wsj()')

ggplot(mtcars) + 
  geom_point(aes(mpg, disp)) +
  theme_tufte()+
  ggtitle('Edward Tufte: theme_tufte()')
```
...
(give an example of a nice plot, then break down each element)