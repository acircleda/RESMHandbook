...
You can use `str()` (from base R) to view how your data is structured. That is, whether variables are numeric, characters, factors. If they are factors, you can also see what their levels are. I recommend using `str()` in the **console** not your script.
...
renamed$age <- 2020-renamed$qyear_born

#Describe experience by age
describeBy(renamed$age, group=renamed$qr_experience, mat=TRUE)
```

What did we do?

  * `renamed$age <-` - Assign whatever is after the `<-` to a new column, "age" in "renamed"
  * `2020-renamed$qyear_born` - Subtract renamed$qyear_born from the current year, 2020.
...
```{r}
table(renamed$qcountry, renamed$qr_experience)
```
...
```
keep <- subset(renamed, select = qr_experience)
dropped <- subset(renamed, select = -qr_experience)
```
...
On the Learning R survey, there was a question about what industry people were in. This resulted in 135 different responses. However, many of these can be combined to make several large groups. First, let's view the jobs.
...
First, what did we do?
  ...
  * `renamed <- ` - saves the following commands to the "renamed" data object
  * `renamed %>%` - means to use the renamed data object "and then"
...
# and check
renamed %>% count(industry)
```
We now have 123 rows. Let's see what we did, and then expand it:

  * `renamed <-` - assign to the "renamed" object
  * `renamed %>%` - use "renamed" and then
...
Let's exand this to categorize as much as possible. To do this efficiently, I will run `renamed$industry` in the *console* and scroll through it as I create categories
...
What did we do?
  ...
  * `renamed <-` - assign the new resulting data to "renamed"
  * `renamed %>%` - use renamed and then
...
I highly doubt these are serious responses, so we would need to deal with them in some way (see [Dealing with Outliers]).
...
```{r}
plot(age ~ qr_experience, data=renamed)
```
...
What did we do?
  ...
  * `data=renamed)` - from the "renamed" data frame
...
```{r}
renamed$z_of_age <- (renamed$age - mean(renamed$age, na.rm=T))/sd(renamed$age, na.rm = T)

# to check
summary(renamed$z_of_age)
```

But, we could also load a package and use a function to do it quickly. For example, the `psycho` package has a `standardize()` function.

```{r message=FALSE, warning=FALSE}
# install package if needed
# install.packages("psycho")
library(psycho)
renamed$z_of_age <- standardize(renamed$age)
...
```{r}
filtered <- subset(renamed, age < 99)
plot(filtered$age)
```
...
```{r}
plot(density(renamed$age, na.rm=T))
```
...
skewness(renamed$age, na.rm = T)
kurtosis(renamed$age, na.rm = T)
```
...
```{r}
  mean(complete.cases(renamed$age))
```
...
```{r}
missing_dropped_some <- renamed %>%
  drop_na(age)
```

This will remove all rows with misisng data in the age variable.
Compared "renamed" to "missing_dropped_some" and you will see less observations.
...
```{r}
mean(renamed$age, na.rm=T) #note: you must remove NAs for this to run
```
