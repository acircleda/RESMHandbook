...
It uses uses survey data from the [RStudio Learning R Survey](https://github.com/rstudio/learning-r-survey). You can access this data, which includes all transformations and cleanings from Chapter 2, [here]("data/rsurvey.RData").
...
Long data is often referred to as tabular data and is more machine-readible than human-readible. It is also very similar to [Tidy data][Introduction to the Tidyverse].
...
We can connect the second data frame to the main data frame by joining on the column they have in common, "name". This will create a single, merged data frame as follows:
...
We have created a data frame called "rsurvey_check_all," which contains all our check-all questions and whether someone checked it (indicated by 1) or did not check it (indicated by 2). We can combined this with our original "rsurvey" data like this:
